...
class LawCaseFinder {
    ...
    setupEventListeners() {
        ...
        // Auto-generate delay input - update config in real-time
        const autoGenerateDelayInput = document.querySelector('input[name="autoGenerateDelay"]');
        if (autoGenerateDelayInput) {
            autoGenerateDelayInput.addEventListener('change', (e) => {
                this.config.autoGenerateDelay = parseFloat(e.target.value) || 0.8;
                this.saveConfig();
                this.consoleLog(`Auto-generate delay set to ${this.config.autoGenerateDelay}s`, 'info');
            });
        }
    }
    ...
    showSection(sectionName) {
        ...
        // Hide auto-generate message when switching to upload section
        if (sectionName === 'upload') {
            this.hideAutoGenerateMessage();
        }
    }
    ...
    async checkAutoGenerateBrief() {
        try {
            ...
            if (isAutoGenerateEnabled) {
                ...
                // Get delay from config (convert seconds to milliseconds)
                const delayMs = (this.config.autoGenerateDelay || 0.8) * 1000;
                console.log(`⏱️ Auto-generate delay: ${this.config.autoGenerateDelay}s (${delayMs}ms)`);
                
                // Show auto-generate message with countdown
                this.showAutoGenerateMessage(this.config.autoGenerateDelay || 0.8);
                
                // Configurable delay to let user see extraction results
                setTimeout(() => {
    ...
    applyPreferencesToUI(preferences) {
        ...
        if (preferences.general) {
            ...
            const autoDelay = document.querySelector('input[name="autoGenerateDelay"]');
            if (autoDelay) autoDelay.value = preferences.general.auto_generate_delay || 0.8;
        }
        ...
        if (preferences.general) {
            this.config.autoGenerateBrief = preferences.general.auto_generate_brief || false;
            this.config.autoGenerateDelay = preferences.general.auto_generate_delay || 0.8;
        }
    }
        ...
        try {
            ...
            const preferences = {
                general: {
                    ...
                    auto_generate_delay: parseFloat(document.querySelector('input[name="autoGenerateDelay"]')?.value) || 0.8
                },
                llm: {
            ...
            this.config.autoGenerateDelay = preferences.general.auto_generate_delay;
    ...
    async resetPreferences() {
        ...
        try {
            ...
            this.config.autoGenerateDelay = defaultPreferences.general.auto_generate_delay;
            localStorage.setItem('lawCaseFinderConfig', JSON.stringify(this.config));
    ...
    applyDefaultPreferencesToUI(preferences) {
        ...
        if (preferences.general) {
            ...
            const autoDelayInput = document.querySelector('input[name="autoGenerateDelay"]');
            if (autoDelayInput) autoDelayInput.value = preferences.general.auto_generate_delay || 0.8;
        }
    ...
    // Auto-generate Message Methods
    showAutoGenerateMessage(delaySeconds) {
        const messageEl = document.getElementById('auto-generate-message');
        const countdownEl = document.getElementById('countdown-timer');
        const progressFillEl = document.getElementById('auto-generate-progress-fill');
        
        if (!messageEl || !countdownEl || !progressFillEl) {
            console.warn('Auto-generate message elements not found');
            return;
        }
        ...
        this.startCountdownTimer(delaySeconds, countdownEl, progressFillEl);
    }
    
    hideAutoGenerateMessage() {
        const messageEl = document.getElementById('auto-generate-message');
        if (messageEl) {
    ...
    startCountdownTimer(delaySeconds, countdownEl, progressFillEl) {
        let remainingTime = delaySeconds;
        const totalTime = delaySeconds;
        ...
        this.updateCountdownDisplay(remainingTime, countdownEl, progressFillEl, totalTime);
    ...
    updateCountdownDisplay(remainingTime, countdownEl, progressFillEl, totalTime) {
        // Update countdown text
        countdownEl.textContent = remainingTime.toFixed(1);
